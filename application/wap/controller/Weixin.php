  ...
  /*-----------------------如果是取消关注 unsubscribe 事件--------------------------*/
    ...
    if( strtolower($postObj->MsgType) == 'text'){
      ...
      //如果用户回复消息
		...
		$res1 = db('text')->where("
precisions=1 and keyword='$keyword'")->find();
		$res2 = db('text')->where("
precisions=0 and keyword like '%$keyword%'")->find();
			if($res1){
...
precisions=0 and keyword like '%$keyword%'")->find();
			...
			if($res3){
						...
						db('img')->where('id',$res3['id'])->setInc('click');
					
			$arr[] = array('title'=>$res3['title'],'description'=>$res3['text'],'picUrl'=>$res3['pic'],'url'=>$res3['url']);
			$template = $WechatApi->getTpl('news',$arr);
			$msgType='news';
			}
			if($res4){
				
				       db('img')->where('id',$res4['id'])->setInc('click');
				
			$arr[] = array('title'=>$res4['title'],'description'=>$res4['text'],'picUrl'=>$res4['pic'],'url'=>$res4['url']);
			$template = $WechatApi->getTpl('news',$arr);
			$msgType='news';
				}
		 ...
		 /*$template = $WechatApi->getTpl('news',$content);
